...
class CustomerAdmin(admin.ModelAdmin):
    list_display = ('id','name','qq','source','consultant','content','status','date')  #自定制显示内容
    list_filter = ('source','consultant','date')  ## 过滤器
    search_fields = ('qq','name')  # 搜索字段
    raw_id_fields = ('consult_course',)  #会显示外键的详细信息
    filter_horizontal = ('tags',)   #ManyToMany多对多字段,编辑页面设置
    list_editable = ('status',)  #设置默认可以编辑字段
    # readonly_fields = ('qq','consultant')    #设置不可修改的字段
...
class CourseRecordAdmin(admin.ModelAdmin):
    ...
    def initialize_studyrecords(self,request,queryset):
        ...
        return redirect("http://127.0.0.1:8000/admin/crm/studyrecord/?course_record__id__exact=%s" % queryset[0].id)
